       ...
       public void run() {
           ...
           try {
             ...
             DomText domText;
               ...
               countDownLatch.countDown();//表示该任务结束
           }
       }
   }
...
/**
 * 线程池不会关闭，JVM不会结束，所以利用CountDownLatch来判断任务是否完成，只有任务完成了，才能去手动关闭线程池
 * 主线程阻塞，每执行完一个线程就调用countDown（），此时计数器-1，直到计时器为0时才将主线程唤醒，不为0 就一直await()
 */
